...
namespace TestingVGLTU.Data.Repositories
{
    public class TeacherRepository : ITeacherRepository
    {
        ...
        public async Task<Teacher?> GetById(int id) => await _context.Teachers.Include(i => i.Testings).AsNoTracking().Where(s => s.Id == id).FirstOrDefaultAsync();

        public async Task<Teacher?> GetByLogin(string login) => await _context.Teachers.Include(i => i.Testings).AsNoTracking().Where(s => s.Login == login).FirstOrDefaultAsync();
