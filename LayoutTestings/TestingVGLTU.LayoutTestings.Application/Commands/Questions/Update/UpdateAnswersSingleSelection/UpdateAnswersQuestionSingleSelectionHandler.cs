...
public class UpdateAnswersQuestionSingleSelectionHandler : ICommandHandler<Guid, UpdateAnswersQuestionSingleSelectionCommand>
{
    private readonly ILayoutTestingRepository _volunteerRepository;
    private readonly ILogger<UpdateAnswersQuestionSingleSelectionHandler> _logger;
    private readonly IValidator<UpdateAnswersQuestionSingleSelectionCommand> _validator;
    private readonly IUnitOfWork _unitOfWork;

    public UpdateAnswersQuestionSingleSelectionHandler(
        ILayoutTestingRepository volunteerRepository,
        ILogger<UpdateAnswersQuestionSingleSelectionHandler> logger,
        IValidator<UpdateAnswersQuestionSingleSelectionCommand> validator,
    ...
    public async Task<Result<Guid, ErrorList>> Handle(UpdateAnswersQuestionSingleSelectionCommand command, CancellationToken cancellationToken = default)
    {
        ...
        var rightAnswerResult = Answer.Create(command.RightAnswer);
        if (rightAnswerResult.IsFailure)
            return rightAnswerResult.Error.ToErrorList();

        var result = layoutTestingResult.Value.UpdateAnswersQuestionSingleSelection(
            questionId,
            answersOptions,
            rightAnswerResult.Value);
        if (result.IsFailure)
